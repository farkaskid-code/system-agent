...
PERMISSIONS_FILE = Path("src/system_agent/config/tool_permissions.yaml")
...
def check_permissions(tool: Tool) -> dict:
    """
    Check if the current user has permission to run the specified tool.
    """
    logger.debug(f"loading permissions from: {PERMISSIONS_FILE}")
    permissions = safe_load(PERMISSIONS_FILE.read_text())["permissions"]
...
def execute(tool: Tool) -> ToolResult:
    """
    Execute the specified tool in a sandboxed environment with resource limits.
    """
    try:
        current_user_permissions = check_permissions(tool)
